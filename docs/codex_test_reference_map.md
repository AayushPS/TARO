...
| `src/test/java/org/Aayush/routing/future/TemporalFidelityContractTest.java` | cross-phase B→C fidelity coverage for periodic activation windows, recency-sensitive prior shifts, directional asymmetry, aggregate compromise preservation, and route/matrix scenario-order alignment | `@Tag("integration")`; best shared temporal-fidelity contract suite spanning C1-C3 behavior |
| `src/test/java/org/Aayush/routing/future/ScenarioPriorConsistencyContractTest.java` | cross-phase E→C prior-consistency coverage for route/matrix bundle parity and served dominant-scenario explanation alignment | `@Tag("integration")`; shared B6/C1/C4/E4 contract suite |
